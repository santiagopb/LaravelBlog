...
@section('content')
      <div class="row">
          <div class="col-md-12">
              <h3>
                <img src="{{ URL::asset('uploads/avatars/'.$user->avatar) }}" style="width:62px; height:62px; border-radius:50%; margin-right:15px;">
                {{ $user->name }}
              </h3>
          </div><!-- .col-md-12 -->
      </div><!-- .row -->
      ...
      {!! Form::model($user, array('route' => ['client.update', $user->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data')) !!}
      <div class="row">
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            @foreach($roles as $role)
              {{ Form::checkbox('roles[]', $role->name, $user->hasRole($role->name), ['hidden' => 'true'] ) }}
            @endforeach
          </div>
      </div><!-- .row -->
      {{ Form::submit('Editar usuario', ['class' => 'btn btn-sm btn-primary'])}}
      {!! Form::close() !!}
@endsection
